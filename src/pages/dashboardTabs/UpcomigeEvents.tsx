...
export default function UpcomingEvent() {
  ...
  async function fetchTickets(customer: {
    ...
    try {
      ...
      // Agrupar eventos por eventId y filtrar eventos futuros
      ...
      const futureEvents = groupedEvents.filter(event => isUpcomingEvent(event.date));
  ...
  return (
    <div className='space-y-5'>
      {events.length === 0 ? (
        <p className='text-center text-lg font-semibold'>You don't have any upcoming events.</p>
      ) : (
        events.map((event) => (
          <EventClaim
            key={event.id}
