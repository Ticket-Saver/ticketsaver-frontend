...
``
export default function EveClaim({ title, venue, city, thumbnailURL, route = '', description2}: EventsClaimConfig) {
  ...
  return (
    <div className='w-full bg-neutral rounded-xl flex flex-col sm:flex-row'>
      <div className='flex flex-col justify-between p-4 py-6 w-full sm:w-2/5'>
        ...
        <div className='text-lg sm:text-xl pb-4'>{city}, {venue}</div>
        ...
        <Link to={route}>
          <button className='btn btn-primary btn-outline px-4 sm:px-10'>Claim you collectible</button>
        </Link>
      </div>
