import React, {
  ...
  import LogeMap from "@/components/assets/LogeMap";
  import OrchestraMap from "@/components/assets/OrchestraMap";
  import CalifoniaTheatreMap from "@/components/assets/CaliforniaTheatre";
  import AppSyncCalls from "@/config/AppSyncCalls";
  ...
  export default function TicketSelection() {
    ...
    const [eventZoneSelected, setEventZoneSelected] = useState<
      "orchestra" | "loge" | ""
    >("orchestra");
    ...
    const ServiceFees = {
      orchestra: [13.20, 10.80, 9.0, 7.20, 4.8],
    };
    ...
    const ticketCost =
      cart?.reduce((acc, crr) => (acc = acc + crr.price), 0) || 0;
  
    // Assuming a fixed service fee of $5
    const serviceFee =
      cart?.reduce((acc, crr) => (acc = acc + crr.serviceFee), 0) || 0;
    const ffFee = cart?.reduce((acc, crr) => (acc = acc + crr.ffFee), 0) || 0;
    const creditCardFee =
      cart?.reduce((acc, crr) => (acc = acc + crr.creditCardFee), 0) || 0;
    ...
    const [seatchartCurrentOptions, setSeatchartCurrentOptions] =
      useState<any>(null);
    ...
    const handleOnSeatClick = async (e: CartChangeEvent) => {
      ...
      if (e.action === "add") {
        ...
        try {
          ...
          setErrorMessage(
            "Failed to lock the seat. Please reload map by clicking it."
        ...
        try {
          await lockSeats(lockingSeat);
          setCart((prev) =>
            prev?.filter((cart) => cart.seatLabel !== e.seat.label)
          );
        } catch (error) {
    ...
    const onTicketClick = (ticket) => {
      setCart((prev) =>
        prev?.filter(
          (cart) =>
            cart.seatLabel !== ticket.seatLabel &&
            cart.seatType === ticket.seatType
        )
      );
  
      const newSelectedSeats = selectedSeats.filter(seat => seat.seatLabel !== ticket.seatLabel || seat.seatType !== ticket.seatType);
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <th className="text-right font-normal">
                        ...
                        <a className="font-bold"> $40 </a>+{" "}
                        <a className="font-bold">Fee </a>
                      </th>
                    </tr>
                  </tbody>
                </table>
                        ...
                        <div className="items-center flex justify-center">
                          <InteractiveMap
                            handleClickImageZone={async (area) => {
                              ...
                              handleGetAreaSeats(area.title).then(
                                (parsedSeats) => {
                                  let selectedOptions = area.Options;
                                  selectedOptions.map.reservedSeats = parsedSeats;
                                  setSeatchartCurrentOptions(selectedOptions);
                            ...
                            getDefaultMap={CalifoniaTheatreMap}
                            width={
                              windowSize < 768
                                ? (windowSize * 6) / 8
                                : windowSize / 2
                            }
                            src={"/maps/CALIFORNIA THETRE.svg"}
                          />
                        </div>
                      </>
                    }
                    ...
                    {cart?.map((ticket, index) => {
                      ...
                      return (
                        ...
                        >
                          <div>
                            <a className="pr-5">
                            Ticket {ticket.subZone} - {ticket.seatLabel.includes('♿') ? `${ticket.seatLabel} - ADA` : ticket.seatLabel.includes('🚹') ? `${ticket.seatLabel} - COMP` : ticket.seatLabel.includes('🦽') ?`${ticket.seatLabel} - Limited Mobility Seat`: ticket.seatLabel}{" "}
                            </a>
                            <button className="btn btn-circle bg-red-700 text-white"
                              onClick={() => {
                                const sessionIdCookie = getCookie("sessionId");
                                const lockingSeat = { Seat: ticket.seatLabel, row: ticket.coords.row, col: ticket.coords.col, subZone: seatchartCurrentArea.title, sessionId: sessionIdCookie };
