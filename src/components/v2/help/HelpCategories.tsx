...
const CategoryCard = ({ category }: { category: HelpCategory }) => {
  const content = (
    <GlassCard
      ...
      radius='lg'
      hoverable
      className={cn('p-5 h-full block transition', category.href && 'cursor-pointer')}
    >
      <div
        aria-hidden
        className='h-10 w-10 rounded-glass-sm grid place-items-center text-white mb-3'
        style={{
          background: 'linear-gradient(135deg, rgba(212,168,240,0.30), rgba(181,123,232,0.18))'
        }}
      >
        {category.icon}
      </div>
      ...
      <p className='mt-1.5 text-[12.5px] text-white/65 leading-relaxed'>{category.description}</p>
  ...
  if (category.href) {
    return (
      <a href={category.href} className='block focus-visible:outline-none rounded-glass-lg'>
        {content}
      </a>
    )
  }
