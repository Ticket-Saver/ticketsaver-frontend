...
export default function TeamGrid({ members }: TeamGridProps) {
  return (
    <div className='grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-3'>
      {members.map((m) => (
        <GlassCard key={m.name} depth='sm' radius='md' className='p-3 text-center'>
          <div
          ...
          <div className='mt-2 text-[12px] text-white font-semibold truncate'>{m.name}</div>
          <div className='text-[10px] text-white/50 mt-0.5 truncate'>{m.role}</div>
        </GlassCard>
      ))}
    </div>
  )
