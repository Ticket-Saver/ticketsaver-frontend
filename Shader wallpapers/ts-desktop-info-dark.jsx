    ...
    <DesktopChrome active="about" onNav={onNav}>
      ...
      <div style={{ padding:'40px 56px 80px', maxWidth:1200, margin:'0 auto' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:18 }}>
          {[['Dani','Founder','#FFB1C8'],['Luca','Engineering','#D4A8F0'],['Mara','Design','#525258'],['Tomás','Ops','#FF5E9E']].map(([n, r, c]) => (
            <div key={n} style={{ padding:28, borderRadius:18, background:'rgba(255,255,255,0.04)', border:'0.5px solid rgba(255,255,255,0.10)', textAlign:'center' }}>
              <div style={{ width:84, height:84, borderRadius:99, margin:'0 auto', background:`linear-gradient(135deg, ${c}, #3D3D43)`, display:'grid', placeItems:'center', color:'#fff', fontFamily:'"Space Grotesk", system-ui', fontSize:30, fontWeight:700, border:'1.5px solid rgba(255,255,255,0.2)' }}>{n[0]}</div>
      ...
      <div style={{ padding:'140px 56px 80px', maxWidth:1100, margin:'0 auto' }}>
        ...
        <div style={{ marginTop:60, display:'grid', gridTemplateColumns:'1fr 1.4fr', gap:48 }}>
          <div>
            ...
            <div style={{ marginTop:20, display:'flex', flexWrap:'wrap', gap:6 }}>
              {['Buying','Refunds','Wallet','Press','Promote','Other'].map((t, i) => (
                <div key={t} style={{ padding:'8px 13px', borderRadius:99, background: i === 0 ? '#fff' : 'rgba(255,255,255,0.05)', color: i === 0 ? '#141416' : '#fff', border:'0.5px solid ' + (i === 0 ? 'transparent' : 'rgba(255,255,255,0.10)'), fontSize:12, fontWeight:600, cursor:'pointer' }}>{t}</div>
              ))}
            </div>
          </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <button style={{ appearance:'none', border:0, cursor:'pointer', marginTop:8, padding:'16px', borderRadius:14, background:'#fff', color:'#141416', fontFamily:'"Space Grotesk", system-ui', fontSize:14, fontWeight:600 }}>Send message →</button>
          </div>
        </div>
      </div>
      <DesktopFooter onNav={onNav}/>
...
function DContactCard({ icon, title, sub, cta }){
  return (
    <div style={{ padding:'22px 24px', borderRadius:18, background:'rgba(255,255,255,0.05)', border:'0.5px solid rgba(255,255,255,0.10)', backdropFilter:'blur(14px) saturate(160%)', display:'flex', alignItems:'center', gap:16 }}>
      <div style={{ width:48, height:48, borderRadius:14, background:'rgba(212,168,240,0.18)', border:'0.5px solid rgba(212,168,240,0.30)', display:'grid', placeItems:'center', color:'#E5E5EA', flexShrink:0 }}>
      ...
      <div style={{ padding:'140px 56px 80px', maxWidth:1100, margin:'0 auto' }}>
        ...
        <div style={{ marginTop:14, display:'flex', flexWrap:'wrap', gap:6 }}>
          {['All','Buying','Wallet & NFT','Refunds','At the door','Selling'].map((c, i) => (
            <div key={c} style={{ padding:'8px 13px', borderRadius:99, background: i === 0 ? '#fff' : 'rgba(255,255,255,0.05)', color: i === 0 ? '#141416' : '#fff', border:'0.5px solid ' + (i === 0 ? 'transparent' : 'rgba(255,255,255,0.10)'), fontSize:12, fontWeight:600, cursor:'pointer' }}>{c}</div>
          ))}
        </div>
            ...
            return (
              <div key={i} onClick={() => setOpen(isOpen ? -1 : i)} style={{ padding:'20px 22px', borderRadius:16, background:'rgba(255,255,255,0.04)', border:'0.5px solid ' + (isOpen ? 'rgba(212,168,240,0.30)' : 'rgba(255,255,255,0.10)'), cursor:'pointer' }}>
                <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                  ...
                  <div style={{ width:28, height:28, borderRadius:99, background: isOpen ? '#D4A8F0' : 'rgba(255,255,255,0.06)', border:'0.5px solid rgba(255,255,255,0.12)', display:'grid', placeItems:'center', color: isOpen ? '#141416' : '#fff', transition:'all 0.2s' }}>
        ...
        <div style={{ marginTop:48, padding:'28px 32px', borderRadius:20, background:'linear-gradient(135deg, rgba(60,60,68,0.22), rgba(255,177,200,0.10))', border:'0.5px solid rgba(255,255,255,0.12)', display:'flex', alignItems:'center', gap:20 }}>
          <div style={{ flex:1 }}>
          ...
          <div style={{ padding:'12px 20px', borderRadius:99, background:'#fff', color:'#141416', fontSize:13.5, fontWeight:600, cursor:'pointer' }}>Open chat →</div>
        </div>
      </div>
      <DesktopFooter onNav={onNav}/>
    </DesktopChrome>
