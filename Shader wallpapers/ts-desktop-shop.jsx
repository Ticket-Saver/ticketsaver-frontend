...
const D = { bg: '#0A0418', purple: { hi:'#D4A8F0', mid:'#7C5BC4', lo:'#5B3FA8' } };
  ...
  return (
    <div style={{ position:'relative', width:'100%', height:'100%', overflow:'hidden', background:D.bg, color:'#fff', fontFamily:'Inter, system-ui' }}>
      ...
      <div style={{
        ...
        background:'linear-gradient(180deg, rgba(10,4,24,0.7), transparent)',
      }}>
        <div onClick={() => onNav('home')} style={{ display:'flex', alignItems:'center', gap:10, cursor:'pointer' }}>
          <div style={{ width:34, height:34, borderRadius:9, background:`linear-gradient(135deg, ${D.purple.hi}, ${D.purple.mid})`, display:'grid', placeItems:'center', boxShadow:`0 4px 14px ${D.purple.mid}80` }}>
            <svg width="16" height="16" viewBox="0 0 14 14" fill="none">
              <path d="M3 4 L7 2 L11 4 L11 10 L7 12 L3 10 Z" stroke="#0E0820" strokeWidth="1.6" strokeLinejoin="round"/>
        ...
        <div onClick={() => onNav('cart')} style={{ position:'relative', cursor:'pointer' }}>
          ...
          {cartCount > 0 && (
            <div style={{ position:'absolute', top:-3, right:-3, minWidth:18, height:18, padding:'0 5px', borderRadius:10, background:'linear-gradient(135deg,#FF5E9E,#B57BE8)', color:'#fff', fontSize:10, fontWeight:700, display:'grid', placeItems:'center', border:'2px solid #0A0418' }}>{cartCount}</div>
          )}
        </div>
  ...
  return (
    <DesktopChrome active="home" onNav={onNav}>
      ...
      <div style={{ position:'relative', height:720, overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', inset:0, background:'linear-gradient(90deg, rgba(10,4,24,0.85) 0%, rgba(10,4,24,0.4) 40%, transparent 70%)' }}/>
        <div style={{ position:'absolute', inset:0, background:'linear-gradient(0deg, rgba(10,4,24,0.95) 0%, transparent 50%)' }}/>
        <div style={{ position:'absolute', left:56, right:56, bottom:80, maxWidth:680 }}>
          ...
          <div style={{ marginTop:32, display:'flex', alignItems:'center', gap:14 }}>
            <button onClick={() => onNav('event')} style={{ appearance:'none', border:0, cursor:'pointer', padding:'15px 28px', borderRadius:99, background:'#fff', color:'#1A0F33', fontFamily:'"Space Grotesk", system-ui', fontSize:15, fontWeight:600, boxShadow:'0 12px 30px rgba(255,255,255,0.15)' }}>Get tickets · From ${cur.priceFrom}</button>
  ...
  return (
    <DesktopChrome active="events" onNav={onNav}>
      ...
      <div style={{ position:'relative', height:560, overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', inset:0, background:'linear-gradient(180deg, rgba(10,4,24,0.6) 0%, rgba(10,4,24,0.2) 40%, rgba(10,4,24,0.98) 100%)' }}/>
        <div style={{ position:'absolute', left:56, right:56, bottom:60, display:'flex', alignItems:'flex-end', justifyContent:'space-between' }}>
          <div style={{ maxWidth:760 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
              ...
              <div style={{ padding:'5px 12px', borderRadius:99, background:'rgba(212,168,240,0.18)', border:'0.5px solid rgba(212,168,240,0.30)', fontSize:10.5, color:'#E0C0FF', fontWeight:700, letterSpacing:'0.14em' }}>EAST COAST TOUR</div>
            </div>
      ...
      <div style={{ padding:'40px 56px 80px', display:'grid', gridTemplateColumns:'1fr 380px', gap:48 }}>
        ...
        <div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:10 }}>
              {dates.map((d, i) => (
                <div key={i} onClick={() => setSel(i)} style={{
                  ...
                  <div style={{ fontSize:10.5, color:i===sel?'#E0C0FF':'rgba(255,255,255,0.55)', fontWeight:700, letterSpacing:'0.10em' }}>{d.day.toUpperCase()} · JUN</div>
          ...
          <div style={{ marginBottom:36 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr 1fr', gridTemplateRows:'repeat(2, 180px)', gap:10 }}>
              <div style={{ gridRow:'span 2', position:'relative', borderRadius:14, overflow:'hidden', background:`linear-gradient(135deg, ${D.purple.lo}, #1A0F33)` }}>
                ...
                <div style={{ position:'absolute', inset:0, display:'grid', placeItems:'center' }}>
                  <div style={{ width:60, height:60, borderRadius:99, background:'rgba(255,255,255,0.95)', display:'grid', placeItems:'center' }}>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="#1A0F33"><path d="M6 4l10 6-10 6V4Z"/></svg>
                  </div>
                </div>
          ...
          <div>
            ...
            <div style={{ position:'relative', height:300, borderRadius:18, overflow:'hidden', border:'0.5px solid rgba(255,255,255,0.12)' }}>
              ...
              />
              <div style={{ position:'absolute', top:14, left:14, padding:'8px 12px', borderRadius:12, background:'rgba(10,4,24,0.85)', border:'0.5px solid rgba(255,255,255,0.12)', backdropFilter:'blur(14px)' }}>
        ...
        <div>
          <div style={{
            ...
            <button onClick={() => onNav('section')} style={{ marginTop:18, width:'100%', appearance:'none', border:0, cursor:'pointer', padding:'15px', borderRadius:14, background:'#fff', color:'#1A0F33', fontFamily:'"Space Grotesk", system-ui', fontSize:14, fontWeight:600 }}>Choose seats →</button>
    ...
    <DesktopChrome active="events" onNav={onNav}>
      <div style={{ padding:'120px 56px 60px', display:'grid', gridTemplateColumns:'1fr 360px', gap:48 }}>
        ...
        <div>
          ...
          {/* Venue SVG */}
          <div style={{ position:'relative', height:560, borderRadius:24, overflow:'hidden', background:'radial-gradient(ellipse at top, rgba(212,168,240,0.10), rgba(10,4,24,0.6))', border:'0.5px solid rgba(255,255,255,0.10)' }}>
              ...
              {sections.map((s, i) => {
                ...
                return (
                  <g key={s.id} onMouseEnter={() => setHover(s.id)} onMouseLeave={() => setHover(null)} onClick={() => onNav('seats')} style={{ cursor:'pointer' }}>
                    ...
                    />
                    <text x={x} y={y+30} fill="#1A0F33" fontSize="14" fontWeight="700" textAnchor="middle" fontFamily='"Space Grotesk"'>SEC {s.label}</text>
                    <text x={x} y={y+50} fill="#1A0F33" fontSize="11" fontWeight="600" textAnchor="middle" opacity={0.7}>{tier.name}</text>
                  </g>
                );
              })}
            </svg>
            ...
            {hover && (
              <div style={{ position:'absolute', bottom:20, left:'50%', transform:'translateX(-50%)', padding:'10px 16px', borderRadius:99, background:'rgba(10,4,24,0.92)', border:'0.5px solid rgba(255,255,255,0.18)', backdropFilter:'blur(14px)', fontSize:13, fontWeight:500 }}>
      ...
      <div style={{ padding:'120px 56px 60px', display:'grid', gridTemplateColumns:'1fr 380px', gap:48 }}>
                  ...
                  {Array.from({ length:14 }, (_, i) => 14 - i).map(n => {
                    ...
                    return (
                      <div key={n} onClick={() => !isTaken && toggle(id)} style={{
                        ...
                        color: isTaken ? 'rgba(255,255,255,0.4)' : isPicked ? '#1A0F33' : '#fff',
        ...
        <div>
          <div style={{ position:'sticky', top:90, padding:24, borderRadius:20, background:'rgba(255,255,255,0.04)', border:'0.5px solid rgba(255,255,255,0.12)', backdropFilter:'blur(18px) saturate(160%)' }}>
            ...
            <button onClick={() => onNav('ticket')} style={{ marginTop:18, width:'100%', appearance:'none', border:0, cursor:'pointer', padding:'15px', borderRadius:14, background:'#fff', color:'#1A0F33', fontFamily:'"Space Grotesk", system-ui', fontSize:14, fontWeight:600 }}>Continue ({picked.size}) →</button>
      ...
      <div style={{ padding:'120px 56px 80px', maxWidth:1100, margin:'0 auto' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1.1fr 1fr', gap:48, alignItems:'center' }}>
          ...
          <div style={{
            ...
            aspectRatio:'5/7',
            borderRadius:24,
            background:`linear-gradient(135deg, #4D1F5F 0%, #1A0830 50%, #2A1050 100%)`,
            overflow:'hidden',
            boxShadow:`0 30px 80px rgba(124,91,196,0.4)`,
            ...
            <div style={{ position:'absolute', left:-10, bottom:118, width:20, height:20, borderRadius:99, background:'#0A0418' }}/>
            <div style={{ position:'absolute', right:-10, bottom:118, width:20, height:20, borderRadius:99, background:'#0A0418' }}/>
            ...
            <div style={{ position:'absolute', left:32, right:32, bottom:32, display:'flex', alignItems:'flex-end', justifyContent:'space-between' }}>
              ...
              <div style={{ width:74, height:74, borderRadius:8, background:'#fff', padding:6, display:'grid', placeItems:'center' }}>
                <div style={{ width:'100%', height:'100%', background:`repeating-conic-gradient(#1A0F33 0 25%, #fff 0 50%)`, backgroundSize:'6px 6px', borderRadius:3 }}/>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <button onClick={() => onNav('mytickets')} style={{ marginTop:18, width:'100%', appearance:'none', border:0, cursor:'pointer', padding:'15px', borderRadius:14, background:'#fff', color:'#1A0F33', fontFamily:'"Space Grotesk", system-ui', fontSize:14, fontWeight:600 }}>View in My Tickets →</button>
          </div>
        </div>
      </div>
...
function ActionRow({ icon, title, sub }){
  return (
    <div style={{ padding:'14px 16px', borderRadius:14, background:'rgba(255,255,255,0.04)', border:'0.5px solid rgba(255,255,255,0.10)', display:'flex', alignItems:'center', gap:14, cursor:'pointer' }}>
      <div style={{ width:36, height:36, borderRadius:10, background:'rgba(212,168,240,0.18)', border:'0.5px solid rgba(212,168,240,0.30)', display:'grid', placeItems:'center', color:'#E0C0FF' }}>
      ...
      <div style={{ padding:'120px 56px 80px' }}>
        ...
        <div style={{ display:'flex', gap:6, marginBottom:28 }}>
          {[['upcoming','Upcoming'],['past','Past'],['collection','Collection']].map(([id, l]) => (
            <div key={id} onClick={() => setTab(id)} style={{ padding:'10px 18px', borderRadius:99, background: id === tab ? '#fff' : 'rgba(255,255,255,0.05)', color: id === tab ? '#1A0F33' : '#fff', fontSize:13, fontWeight:600, cursor:'pointer' }}>{l}</div>
          ))}
        </div>
            ...
            {(tab === 'upcoming' ? upcoming : past).map(e => (
              <div key={e.id} style={{ display:'flex', borderRadius:18, overflow:'hidden', background:'rgba(255,255,255,0.04)', border:'0.5px solid rgba(255,255,255,0.10)', opacity: tab === 'past' ? 0.65 : 1 }}>
                ...
                <div style={{ position:'relative', width:1, background:'rgba(255,255,255,0.10)' }}>
                  <div style={{ position:'absolute', top:-8, left:-8, width:16, height:16, borderRadius:99, background:'#0A0418', border:'0.5px solid rgba(255,255,255,0.10)' }}/>
                  <div style={{ position:'absolute', bottom:-8, left:-8, width:16, height:16, borderRadius:99, background:'#0A0418', border:'0.5px solid rgba(255,255,255,0.10)' }}/>
                </div>
        ...
        {tab === 'collection' && (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:18 }}>
            {[...Array(8)].map((_, i) => {
              ...
              return (
                <div key={i} style={{ borderRadius:18, overflow:'hidden', background:`linear-gradient(135deg, ${D.purple.lo}, #1A0F33)`, border:'0.5px solid rgba(255,255,255,0.10)', aspectRatio:'1/1', position:'relative' }}>
