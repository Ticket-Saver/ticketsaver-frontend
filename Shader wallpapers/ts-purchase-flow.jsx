...
function SeatDots({ x, y, w, h }){
  ...
  return <g pointerEvents="none">{dots.map(([cx, cy], i) => <circle key={i} cx={cx} cy={cy} r={0.7} fill="rgba(26,15,51,0.35)"/>)}</g>;
}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 2, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        background: 'linear-gradient(to top, rgba(10,4,24,0.96) 60%, rgba(10,4,24,0))',
      }}>
        <div style={{
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button disabled={count === 0} style={{
            ...
            color: count ? '#1A0F33' : 'rgba(255,255,255,0.45)',
            fontFamily: '"Space Grotesk", system-ui',
            fontSize: 13, fontWeight: 600,
  ...
  return (
    <div style={{
      ...
      <svg viewBox="0 0 21 21" style={{ display: 'block' }}>
        {cells.map(([x, y], i) => <rect key={i} x={x} y={y} width={1} height={1} fill="#1A0F33"/>)}
      </svg>
    </div>
  );
}
