    ...
    <div style={{ display:'flex', gap:4, padding:3, borderRadius:99, background:'rgba(0,0,0,0.5)', border:'0.5px solid rgba(255,255,255,0.14)', backdropFilter:'blur(14px)' }}>
      {Object.keys(STATES).map(k => (
        <div key={k} onClick={() => setState(k)} style={{
          ...
          color: state === k ? '#141416' : 'rgba(255,255,255,0.7)',
...
function QueueMobile(){
  ...
  return (
    <div style={{ width:'100%', height:'100%', position:'relative', background:'#0A0A0C', overflow:'hidden', color:'#fff', fontFamily:'Inter, system-ui' }}>

      {/* Event cover background, heavily blurred */}
      <div style={{
        position:'absolute', inset:0,
        backgroundImage: event.cover ? `url(${event.cover})` : 'linear-gradient(135deg, #1F1F22, #3D3D43)',
        backgroundSize:'cover', backgroundPosition:'center',
        filter:'blur(40px) saturate(140%) brightness(0.45)',
        transform:'scale(1.2)',
      }}/>
      <div style={{ position:'absolute', inset:0, background:'linear-gradient(180deg, rgba(10,10,12,0.20) 0%, rgba(10,10,12,0.85) 100%)' }}/>
      ...
      <div style={{ position:'relative', margin:'14px 20px 0', padding:'10px 12px', display:'flex', alignItems:'center', gap:11, borderRadius:14, background:'rgba(255,255,255,0.06)', border:'0.5px solid rgba(255,255,255,0.10)', backdropFilter:'blur(20px)' }}>
        <div style={{ width:42, height:42, borderRadius:10, backgroundImage: event.cover ? `url(${event.cover})` : 'linear-gradient(135deg, #D4A8F0, #525258)', backgroundSize:'cover', flexShrink:0 }}/>
...
function QueueDesktop({ onNav }){
  ...
  return (
    <div style={{ width:'100%', height:'100%', minHeight:900, position:'relative', background:'#0A0A0C', overflow:'hidden', color:'#fff', fontFamily:'Inter, system-ui' }}>
      ...
      <div style={{ position:'absolute', inset:0, background:'linear-gradient(180deg, rgba(10,10,12,0.30) 0%, rgba(10,10,12,0.92) 100%)' }}/>
      ...
      <div style={{ position:'relative', height:72, padding:'0 56px', display:'flex', alignItems:'center', gap:16 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:26, height:26, borderRadius:7, background:'linear-gradient(135deg,#D4A8F0,#525258)' }}/>
      ...
      <div style={{ position:'relative', padding:'40px 56px 56px', display:'grid', gridTemplateColumns:'440px 1fr', gap:48, alignItems:'start' }}>
        ...
        <div style={{ borderRadius:24, overflow:'hidden', background:'rgba(255,255,255,0.05)', border:'0.5px solid rgba(255,255,255,0.10)', backdropFilter:'blur(20px) saturate(160%)' }}>
          <div style={{ height:300, backgroundImage: event.cover ? `url(${event.cover})` : 'linear-gradient(135deg, #FFB1C8 0%, #525258 50%, #1F1F22 100%)', backgroundSize:'cover', backgroundPosition:'center' }}/>
        ...
        {state !== 'released' ? (
          <div>
            ...
            <div style={{ marginTop:40, padding:20, borderRadius:16, background:'rgba(255,255,255,0.04)', border:'0.5px solid rgba(255,255,255,0.10)' }}>
              ...
              <div style={{ height:6, borderRadius:99, background:'rgba(255,255,255,0.08)', overflow:'hidden' }}>
                <div style={{ width:`${s.pct * 100}%`, height:'100%', background:`linear-gradient(90deg, #525258, ${s.color})`, transition:'width 0.6s' }}/>
              </div>
            </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:600, textAlign:'center' }}>
            ...
            <div onClick={() => onNav && onNav('seats')} style={{ marginTop:40, padding:'16px 28px', borderRadius:99, background:'#fff', color:'#141416', fontFamily:'"Space Grotesk", system-ui', fontSize:14, fontWeight:600, cursor:'pointer' }}>Skip to seat picker →</div>
            <style>{`@keyframes spin { to { transform: rotate(360deg) } }`}</style>
          </div>
        )}
      </div>
